...
export const search = async (values, sortBy) => {
  const response = await fetch(
    `https://api.react-learning.ru/products/search?query=${values}`,
    {
      method: "GET",
      headers: {
  ...
  if (sortBy == "sortByAscPrice") {
    result.sort((a, b) => {
      return (
        optionalPrice(a.price, a.discount) - optionalPrice(b.price, b.discount)
      );
    });
  }
  ...
  if (sortBy == "sortByDescPrice") {
    result.sort((a, b) => {
      return (
        optionalPrice(b.price, b.discount) - optionalPrice(a.price, a.discount)
      );
    });
  }
...
export const editAvatar = async (avatar) => {
  const res = await fetch(
    `https://api.react-learning.ru/v2/9-gr/users/me/avatar`,
    {
      method: "PATCH",
      headers: {
        Accept: "application/json",
...
export const getUser = async (userID) => {
  const response = await fetch(
    `https://api.react-learning.ru/v2/9-gr/users/${userID}`,
    {
      method: "GET",
      headers: {
        Accept: "application/json",
...
export const getReviewsOneProduct = async (productID) => {
  let response = await fetch(
    `https://api.react-learning.ru/products/review/${productID}`,
    {
      method: "GET",
      headers: {
        Accept: "application/json",
